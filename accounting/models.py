...
class Account(models.Model):
	...
	def real_balance(self):
		...
		total = self.balance
		total += sum(t.amount for t in Transaction.objects.filter(debit=self).filter(cleared=false))
		total -= sum(t.amount for t in Transaction.objects.filter(credit=self).filter(cleared=false))
		if self.is_credit():
			total = -total
		return total

	def is_credit(self):
		return self.account_type in ('CL', 'CE')
